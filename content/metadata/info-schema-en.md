...
| additional_notes | Note | General information relating to an asset, in English | No | fluent_text.html |  | fluent_text |  |
...
| place_of_publication | Place of Publication | The region where the asset was originally published. You may select or deselect multiple options (Ctrl+Click) | No | multiple_select.html | canada_geographic_region | scheming_multiple_choice | choices=310 |
...
| issuance | Issuance | A term that designates how the asset is issued | No | select.html |  | scheming_required scheming_choices | choices=6 |
| issuance_frequency | Frequency of Issuance | A statement of publication frequency in textual form | No | select.html |  | scheming_required scheming_choices | choices=17 |
...
| date_created | Date Created | The date of creation of the asset (YYYY-MM-DD) | No | date.html | date | scheming_required isodate convert_to_json_if_date |  |
| date_captured | Date Captured | The date on which the asset was digitized or a subsequent snapshot was taken (YYYY-MM-DD) | No | date.html | date | scheming_required isodate convert_to_json_if_date |  |
| edition | Edition | Information identifying the version of the asset | No | API only |  |  |  |
| series_publication_dates | Series Dates of Publication and/or Sequential Designation | The beginning/ending date(s) of publication and/or sequential designations used on items in a series, in English | No | fluent_text.html | fluent_text | fluent_text |  |
...
| isbn | ISBN | The identification of the asset’s - International Standard Book Number, a unique identifier assigned to many types of publications except serials by a registration agency | No | API only |  |  |  |
| issn | ISSN | The identification of the asset’s International Standard Serial Number, a unique identifier assigned to serial publications by a registration agency | No | API only |  |  |  |
| gc_catalogue_number | Government of Canada Catalogue Number | The identification of the asset’s  GC Catalogue Number | No | API only |  |  |  |
| departmental_identification_number | Departmental Identification Number | The identification number of the organization which published the asset | No | API only |  |  |  |
| pspc_identification_number | PSPC System Identifier | The identification number given to the asset by Public Service and Procurement Canada | No | API only |  |  |  |
...
| date_published | Date Published | Date of issuance of the resource (e.g. publication) | No | date.html | canada_resource_date_published | scheming_required |  |
...
### `issuance_frequency` (Dataset fields)
