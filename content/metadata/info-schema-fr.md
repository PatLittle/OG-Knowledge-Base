# Schéma Jeu de données (info)

Source: `https://open.canada.ca/data/api/action/scheming_dataset_schema_show?type=info&expanded=true`

Si **Extrait de formulaire** est vide, ce champ est **API seulement**.

## Champs du jeu de données (46)
...
| additional_notes | Note | Information générale concernant un produit, en français | Non | fluent_text.html |  | fluent_text |  |
...
| place_of_publication | Endroit de publication | La région où le produit a été publié initialement. Vous pouvez sélectionner ou désélectionner de multiples options (Ctrl+Clic) | Non | multiple_select.html | canada_geographic_region | scheming_multiple_choice | choices=310 |
...
| issuance | Publication | Un terme qui décrit comment la ressource est publié | Non | select.html |  | scheming_required scheming_choices | choices=6 |
| issuance_frequency | Fréquence de publication | Représentation textuelle de la fréquence de publication | Non | select.html |  | scheming_required scheming_choices | choices=17 |
...
| date_created | Date de création | Date de création du produit (AAAA-MM-JJ) | Non | date.html | date | scheming_required isodate convert_to_json_if_date |  |
| date_captured | Date de saisie | Date à laquelle le produit a été numérisé ou un instantané subséquent a été pris (AAAA-MM-JJ) | Non | date.html | date | scheming_required isodate convert_to_json_if_date |  |
| edition | Edition | Information qui identifie la version du produit | Non | API seulement |  |  |  |
| series_publication_dates | Series Dates of Publication and/or Sequential Designation | Dates de début/fin de la publication et/ou de désignation séquentielle utilisées pour les articles d’une série, en français | Non | fluent_text.html | fluent_text | fluent_text |  |
...
| isbn | ISBN | The identification of the asset’s - International Standard Book Number, a unique identifier assigned to many types of publications except serials by a registration agency | Non | API seulement |  |  |  |
| issn | ISSN | Identification du Numéro international normalisé des publications en série du produit, un identificateur unique assigné aux publications en série par un organisme d’enregistrement | Non | API seulement |  |  |  |
| gc_catalogue_number | Numéro de catalogue du gouvernement du Canada | Identification du numéro du produit dans le catalogue du GC | Non | API seulement |  |  |  |
| departmental_identification_number | Numéro d’identification du ministère | Numéro d’identification de l’organisation qui a publié le produit | Non | API seulement |  |  |  |
| pspc_identification_number | Identificateur de système de SPAC | Le numéro d’identification attribué au produit par Services publics et Approvisionnement Canada | Non | API seulement |  |  |  |
...
| date_published | Date de publication | Date of issuance of the resource (e.g. publication) | Non | date.html | canada_resource_date_published | scheming_required |  |
...
### `issuance` (Champs du jeu de données)
...
### `issuance_frequency` (Champs du jeu de données)
